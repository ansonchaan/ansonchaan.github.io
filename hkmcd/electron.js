...
const logToFile = async (args) => {
  const time = new Date();
  const timestp = time.toISOString().substring(0,10);
  await fileSystem.appendFilePromise(path.join(app.getPath('appData'), `${appName}-data`, `${appName} ${timestp}.csv`), `${time},${args}` + "\r\n");
}
